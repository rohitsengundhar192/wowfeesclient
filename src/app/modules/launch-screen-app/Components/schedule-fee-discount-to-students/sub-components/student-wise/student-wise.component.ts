...
import { RadioBtnComponent } from '../../popup/radio-btn/radio-btn.component';
import { StudentDiscountEditFeeComponent } from '../../popup/student-discount-edit-fee/student-discount-edit-fee.component';
import { StudentCustomEditFeeComponent } from '../../popup/student-custom-edit-fee/student-custom-edit-fee.component';
...
export class StudentWiseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._datashare.share_category_schedule_new_student_Data.subscribe(
      (res) => {
        console.log(res, 'res');
  ...
  getScheduleNewFeeDiscountCategoryTable() {
    this._apiservice
      .getStudentWiseTable(this.country_code, this.customer_id)
      .subscribe((res) => {
        if (res != undefined) {
          let tableData: any[] = [];
          if (res) {
            for (let i = 0; i < res.data.length; i++) {
              ...
              let pushFeeCategoryName: any[] = [];
              let pushDiscountCategoryName: any[] = [];
              this._apiservice
                .getFeeCategoryName(
                  this.country_code,
                  this.customer_id,
                  datass.tableData.fee_category_id
                )
                .subscribe((res) => {
                  pushFeeCategoryName.push(res?.data[0]?.fee_category_name);
                });
              ...
              this._apiservice
                .getFeeDiscountCategoryName(
                  this.country_code,
                  this.customer_id,
                  datass.tableData.discount_category_id
                )
                .subscribe((res) => {
                  pushDiscountCategoryName.push(
                    res?.data[0]?.discount_category_name
                  );
                });

              let datas = {
                datass,
                fee_category_name: pushFeeCategoryName,
                discount_category_name: pushDiscountCategoryName,
              };
  ...
  radioBtnData(e: any) {
    ...
    this.student_wise_schedule_id = e.datass.tableData.student_wise_schedule_id;
  }
  scheduleEditDialogue() {
    if (this.radioRowValue.fee_category_name[0] != undefined) {
      this.scheduleEditDialogueCustom();
    } else {
      this.scheduleEditDialogueDiscount();
    }
  }
  scheduleOpenPopup() {
    const dialogRef = this.dialog.open(RadioBtnComponent, {
      disableClose: true,
      height: '50%',
      width: '400px',
      minWidth: '250px',
  ...
  scheduleEditDialogueDiscount() {
    const dialogRef = this.dialog.open(StudentDiscountEditFeeComponent, {
      disableClose: true,
      height: '60%',
      width: '600px',
      minWidth: '250px',
  ...
  scheduleEditDialogueCustom() {
    const dialogRef = this.dialog.open(StudentCustomEditFeeComponent, {
      disableClose: true,
      height: '60%',
      width: '600px',
      minWidth: '250px',
