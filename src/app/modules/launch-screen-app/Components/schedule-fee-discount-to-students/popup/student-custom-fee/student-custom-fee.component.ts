...
@Component({
  selector: 'app-student-custom-fee',
  templateUrl: './student-custom-fee.component.html',
  styleUrls: ['./student-custom-fee.component.scss'],
})
export class StudentCustomFeeComponent implements OnInit {
  //* --------------------------  Start  -----------------------------------*//
  ...
  //* ---------------------------  Constructor  ----------------------------*//
  constructor(
    public loginDialogRef: MatDialogRef<StudentCustomFeeComponent>,
      ...
      .subscribe({
        next: (_res: any) => {
          if (_res.statusCode == 200) {
            this._snackbar.success(_res.message);
            this.onNoClick();
            this._datashare.passCloseIDDialogueScheduleNewData(1);
          } else {
            this._snackbar.error(_res.message);
          }
        },
