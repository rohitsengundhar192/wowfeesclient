...
import { EditBusFeeComponent } from './popup/edit-bus-fee/edit-bus-fee.component';
import { HistoryBusFeeComponent } from './popup/history-bus-fee/history-bus-fee.component';
import { ApiService } from 'src/app/shared/services/api/api.service';
import { DataSharingService } from 'src/app/shared/services/data-sharing/data-sharing.service';

@Component({
  selector: 'app-bus-fee-management',
  templateUrl: './bus-fee-management.component.html',
  styleUrls: ['./bus-fee-management.component.scss'],
})
export class BusFeeManagementComponent implements OnInit {
  //* --------------------------  Start  -----------------------------------*//
  ...
  //* -------------------------  Lifecycle Hooks  --------------------------*//
  ngOnInit(): void {
    this._headerTitle.setTitle('Bus Fee Management');
  ...
  getTableData() {
    this._apiservice
      .getRoutes(this.country_code, this.customer_id, this.user_id_login)
      .subscribe((res) => {
  ...
  radioBtnData(e: any) {
    ...
    this._datashare.shareDataStopDetails(e.stop_details);
  }
  OpenEditBusFee() {
    const dialogRef = this.dialog.open(EditBusFeeComponent, {
      disableClose: true,
      height: '50%',
      width: '360px',
      minWidth: '250px',
    ...
    dialogRef.afterClosed().subscribe((result) => {
      this.getTableData();
      this._datashare.shareDataStopDetails([]);
      this.btndisable = true;
    });
  }
  OpenhistoryBusFee() {
    const dialogRef = this.dialog.open(HistoryBusFeeComponent, {
      disableClose: true,
      height: '50%',
      width: '560px',
      minWidth: '250px',
